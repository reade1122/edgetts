...
const LAST_PAGE_KEY_PREFIX = "pdfjs.lastPage:";

function isLikelyGoogleDriveFileIdParam(file) {
  if (!file || typeof file !== "string") {
    return false;
  }
  if (file === IDB_LAST_FILE_URL_MARKER) {
    return false;
  }
  ...
  return /^[a-zA-Z0-9_-]{20,}$/.test(file);
}

function getPageStorageKey(file) {
  return `${LAST_PAGE_KEY_PREFIX}${file || ""}`;
}
...
async function saveFileToIndexedDB(file, existingArrayBuffer = null, markSourceCloud = false) {
  const db = await openViewerDB();
  try {
    const buffer =
      existingArrayBuffer != null ? existingArrayBuffer : await file.arrayBuffer();
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    file = params.get("file") ?? localStorage.getItem("pdfjs.lastFile") ?? AppOptions.get("defaultUrl");
    this._lastPageStorageKey = getPageStorageKey(file);
    this._savedPageOnReload = null;
    if (!document.location.hash && file) {
      const savedPage = parseInt(localStorage.getItem(this._lastPageStorageKey), 10);
      if (Number.isInteger(savedPage) && savedPage > 0) {
        this.initialBookmark = `page=${savedPage}`;
        this._savedPageOnReload = savedPage;
      }
    }
    if (file && file !== IDB_LAST_FILE_URL_MARKER) {
    ...
    if (file === IDB_LAST_FILE_URL_MARKER) {
      try {
        ...
        if (saved?.data) {
          ...
          await this.open({
            data,
            originalUrl: saved.name || "IndexedDB PDF"
          });
        } else {
      ...
      if (isLikelyGoogleDriveFileIdParam(file)) {
        this._documentError("pdfjs-missing-file-error", {
          message: "雲端硬碟檔尚未經 GAS 載入（代理失敗、回傳非 JSON，或 viewer-remote-pdf-bootstrap 未執行）。請看主控台，並確認 Apps Script 部署為「知道連結者可存取」。"
        });
      } else if (typeof file === "string" && (file.startsWith("http://") || file.startsWith("https://"))) {
        let networkOpenedAsLocal = false;
        try {
          const absUrl = new URL(file, window.location.href).href;
          const sameOrigin = new URL(absUrl).origin === window.location.origin;
          ...
          const pdfFile = new File([ab], base, {
            type: "application/pdf"
          });
          await saveFileToIndexedDB(pdfFile, ab, true);
          try {
            localStorage.setItem("pdfjs.lastFile", IDB_LAST_FILE_URL_MARKER);
            const currentUrl = new URL(window.location.href);
            currentUrl.searchParams.set("file", IDB_LAST_FILE_URL_MARKER);
          ...
          networkOpenedAsLocal = true;
        } catch (ex) {
          console.warn("PDFViewerApplication: 網址 PDF 幕後載入改以與本機檔相同方式開啟失敗，改回 URL 串流。", ex);
        }
        if (!networkOpenedAsLocal) {
          await this.open({
            url: file
          });
        }
  ...
  async close() {
    ...
    this._hideViewBookmark();
    if (!this.pdfLoadingTask && !this.pdfDocument) {
      return;
    }
  ...
  async open(args) {
    ...
    this._lastPageStorageKey = getPageStorageKey(args?.url || IDB_LAST_FILE_URL_MARKER);
    const workerParams = AppOptions.getAll(OptionKind.WORKER);
    Object.assign(GlobalWorkerOptions, workerParams);
    if (args.url) {
      this.setTitleUsingUrl(args.originalUrl || args.url, args.url);
      // 記住目前開啟的檔案 URL，重新整理後可自動回到該檔案。
      if (typeof args.url === "string" && !args.url.startsWith("blob:")) {
        try {
          localStorage.setItem("pdfjs.lastFile", args.url);
          const currentUrl = new URL(window.location.href);
          currentUrl.searchParams.set("file", args.url);
          window.history.replaceState({}, "", currentUrl);
        } catch (e) {}
      }
    } else if (args.data) {
      const fn = args.originalUrl || "document.pdf";
      this.url = "";
      this.baseUrl = "";
      this._downloadUrl = "";
      this.setTitle(fn);
  ...
  bindEvents() {
    ...
    eventBus._on("pagesinit", onPagesInitRestorePage.bind(this), opts);
...
}
{
  ...
  var stripSheetQueryParamsForLocalOpen = function (u) {
    u.searchParams.delete("googlesheetid");
    u.searchParams.delete("GoogleSheetId");
    u.searchParams.delete("googleSheetId");
  };
  var onFileInputChange = async function (evt) {
    ...
    try {
      ...
      await saveFileToIndexedDB(file, buf, false);
      try {
        localStorage.setItem("pdfjs.lastFile", IDB_LAST_FILE_URL_MARKER);
        const currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set("file", IDB_LAST_FILE_URL_MARKER);
        stripSheetQueryParamsForLocalOpen(currentUrl);
        window.history.replaceState({}, "", currentUrl);
      } catch (e) {}
      await this.open({
      ...
      console.error("開啟本機 PDF（IndexedDB 或 data 載入）失敗，改以 blob URL 重試：", err);
      try {
        try {
          const u = new URL(window.location.href);
          stripSheetQueryParamsForLocalOpen(u);
...
function onPageChanging({
  ...
  if (this._lastPageStorageKey) {
    try {
      localStorage.setItem(this._lastPageStorageKey, String(pageNumber));
    } catch (e) {}
  }
}
function onPagesInitRestorePage() {
  const savedPage = this._savedPageOnReload;
  if (!Number.isInteger(savedPage) || savedPage <= 0) {
    return;
  }
  const maxPage = this.pagesCount || this.pdfViewer?.pagesCount || 0;
  if (maxPage > 0) {
    const target = Math.min(savedPage, maxPage);
    if (this.pdfViewer.currentPageNumber !== target) {
      this.pdfViewer.currentPageNumber = target;
    }
  }
  this._savedPageOnReload = null;
}
...
function closeSecondaryToolbar(evt) {
  ...
  if (appConfig.secondaryToolbar?.toolbar?.contains(evt.target)) {
    return;
  }
